			...
			if (typeof jQuery === 'function') {	
				...
				var	changeView$ = function() {
					if ($(front).css('opacity') === 1) {
							$(front).stop().animate({opacity : 0}, 1500);	
							behind.style.backgroundImage = bgImage.replace(/(_.{4}_)(.+)(\..{3})/,'$1'+src[i]+'$3');
							$(behind).stop().animate({opacity : 1}, 1500);	
						}
						else {
							front.style.backgroundImage = bgImage.replace(/(_.{4}_)(.+)(\..{3})/,'$1'+src[i]+'$3');
							$(front).stop().animate({opacity : 1}, 1500);	
							$(behind).stop().animate({opacity : 0}, 1500);	
						}
						$(logos[i]).stop().animate({opacity : 0}, 1500);			
						if ( i == max) i = 0;
						else i++;
						$(logos[i]).stop().animate({opacity : 1}, 1500);		
					};
